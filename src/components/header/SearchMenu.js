                ...
                {searchHistory &&
                    ...
                    searchHistory
                        ...
                        .map((result) =>
                            result?.type === "group" ? (
                                <>
                                    <div className="search_group_item hover1" key={result?.group?._id}>
                                        <Link className="flex" to={`/group/${result?.group?._id}`} onClick={() => addToSearchHistoryHandler(result?.group?._id, "group")}>
                                ...
                                <>
                                    ...
                                    <div className="search_user_item hover1" key={result._id}>
                                        <Link className="flex" to={`/profile/${result?.user?._id}`} onClick={() => addToSearchHistoryHandler(result?.user?._id, "user")}>
                ...
                {results.combinedResults &&
                    results.combinedResults.map((result) =>
                        result?.group_name ? (
                            <>
                                {" "}
                                <Link to={`/group/${result._id}`} className="search_group_item hover1" onClick={() => addToSearchHistoryHandler(result._id, "group")} key={result._id}>
                            ...
                            <>
                                {" "}
                                <Link to={`/profile/${result._id}`} className="search_user_item hover1" onClick={() => addToSearchHistoryHandler(result._id, "user")} key={result._id}>
