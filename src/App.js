...
import { postsReducer, notificationsReducer, listMessReducer, listPostSavedReducer } from "./functions/reducers";
...
import { friendspageByBirthday, groupspage, groupdiscoverspage, postgroups } from "./functions/reducers";
...
import CallModalPopup from "./pages/test";
function App() {
    ...
    const openChatWindow = (room) => {
        const existingWindowIndex = openChatWindows.findIndex((window) => window._id === room._id);

        if (existingWindowIndex !== -1) {
            setOpenChatWindows([...openChatWindows.slice(0, existingWindowIndex), ...openChatWindows.slice(existingWindowIndex + 1), room]);
    ...
    //             description: (
    //                 <>
    //                     <CallModalPopup socket={socket} receiver={data.caller} />
    //                 </>
    //             ),
    //         });
    //     });
    ...
    const [{ loadingNotification, errorNotification, notifications }, dispatchNotification] = useReducer(notificationsReducer, {
        loading: false,
        notifications: [],
        error: "",
    });

    const [{ loading: loadingListMess, errorListMes, listMess }, dispatchListMes] = useReducer(listMessReducer, {
        loading: false,
        listMess: [],
        error: "",
    });
    ...
    const getNotifications = async () => {
        try {
            ...
            const { data } = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/getAllNotification`, {
                headers: {
    ...
    const [{ loading: loadingListPostSaved, errorListPostSaved, listPostSaved }, dispatchListPostSaved] = useReducer(listPostSavedReducer, {
        loading: false,
        listPostSaved: [],
        error: "",
    });
    ...
    const getListMess = async () => {
        try {
            ...
            const { data } = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/getListRoomMess`, {
                headers: {
                    Authorization: `Bearer ${user.token}`,
                },
            });
    ...
    const [{ loading: friendsLoading, error: friendsError, dataFriend }, dispatchFriends] = useReducer(friendspage, {
        loading: false,
        dataFriend: [],
        error: "",
    });
    const [{ loading: groupsLoading, error: groupsError, dataGroups }, dispatchGroups] = useReducer(groupspage, {
        loading: false,
        dataGroups: [],
        error: "",
    });

    const [{ loading: discoverGroupsLoading, error: discoverGroupsError, dataDiscoverGroups }, dispatchDiscoverGroups] = useReducer(groupdiscoverspage, {
        loading: false,
        dataDiscoverGroups: [],
        error: "",
    });

    const [{ loading: postGroupsLoading, error: postGroupsError, dataPostGroups }, dispatchPostGroups] = useReducer(postgroups, {
        loading: false,
        dataPostGroups: [],
        error: "",
    });

    const [{ loading: roomMessLoading, error: roomMessError, dataRoomMess }, dispatchRoomMess] = useReducer(roommess, {
        loading: false,
        dataRoomMess: [],
        error: "",
    });
    ...
    const [{ loading: friendsByBirthdayLoading, error: friendsByBirthdayError, dataByBirthday }, dispatchFriendsByBirthday] = useReducer(friendspageByBirthday, {
        loading: false,
        dataByBirthday: [],
        error: "",
    });
                                ...
                                <div className="profile_link_active">
                                    <div className="circle_icon_notification">
                                        ...
                                        <div className="right_bottom_notification">
                                            {reacts.includes(data?.type) ? <img src={`../../../../reacts/${data?.type}.svg`} alt="" /> : <i className={`${data?.type}_icon`}></i>}
                                        </div>
                                    </div>
                                    <p>
                                        <div
    ...
    return (
        <div className={darkTheme && "dark"}>
            ...
            {visibleReact && <ReactPopup user={user} setVisibleReact={setVisibleReact} visibleReact={visibleReact} socket={socket} />}
            {visibleReactComment && <ReactCommentPopup user={user} setVisibleReactComment={setVisibleReactComment} visibleReactComment={visibleReactComment} socket={socket} />}
            {visiblePost && (
                <Post_detail
            ...
            {visiblePhotoDetail && <PhotoDetail visiblePhotoDetail={visiblePhotoDetail} setVisiblePhotoDetail={setVisiblePhotoDetail} />}
            {openChatWindows && (
                <>
            ...
            {report_Profile && <ReportMenu_Profile setReport_Profile={setReport_Profile} report_Profile={report_Profile} />}
            ...
            <Routes>
                ...
                <Route path="/verifi/:emailRegister" element={<VerifiRegister socket={socket} />} />
                <Route path="/groupcall" element={<GroupCall socket={socket} />} />
